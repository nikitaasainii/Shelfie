    ...
    return(
         <div className="inset-0 bg-black/40 fixed flex flex-col items-center justify-center z-50 gap-5">
            <div className="bg-white rounded-3xl p-6 h-auto w-100 flex flex-col gap-4">
                ...
                <button onClick={()=>setStatus("reading")} className={`rounded-full px-2 py-1 border-1 text-sm w-24 font-['Times_New_Roman',_serif]  ${status === "reading" ? "bg-[#FF1FA9] text-white" : "bg-white text-black"}`}>reading</button>
                <button onClick={()=>setStatus("want to read")} className={`rounded-full px-2 py-1 border-1 w-24 text-sm font-['Times_New_Roman',_serif]  ${status === "want to read" ? "bg-[#FF1FA9] text-white" : "bg-white text-black"}`}>want to read</button>
                <button onClick={()=>setStatus("DNF")} className={`rounded-full px-2 py-1 border-1 text-sm w-24 font-['Times_New_Roman',_serif]  ${status === "DNF" ? "bg-[#FF1FA9] text-white" : "bg-white text-black"}`}>DNF</button>
                </div>
                <div className="flex flex-col gap-1">
                <button onClick={()=>setStatus("finished")} className={`rounded-full px-2 py-1 border-1 text-sm w-24 font-['Times_New_Roman',_serif]  ${status === "finished" ? "bg-[#FF1FA9] text-white" : "bg-white text-black"}`}>finished</button>
                <button onClick={()=>setStatus("rereading")} className={`rounded-full px-2 py-1 border-1 text-sm w-24 font-['Times_New_Roman',_serif]  ${status === "rereading" ? "bg-[#FF1FA9] text-white" : "bg-white text-black"}`}>rereading</button>
                </div>
                </div>
                ...
                {[1,2,3,4,5].map((star) => (
                    <button key={star} onClick={() => setRating(star)} className={`text-2xl ${star <= rating ? "text-[#FF1FA9]" : "text-gray-300"}`}>
                        {star <= rating ? "★" : "☆"}
                    </button>
                ))}
                </div>
                <label htmlFor=""className="font-['Times_New_Roman',_serif] text-black text-xl">Notes</label>
                <div className="bg-[#FF1FA9] rounded-3xl p-3 h-58 w-88 flex flex-col items-center gap-1">
